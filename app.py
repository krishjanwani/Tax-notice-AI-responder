...
def create_docx(text):
    ...
    title = doc.add_heading('DRAFT RESPONSE TO INCOME TAX NOTICE', 0)
    title.alignment = 1 # Center aligned
    
    doc.add_paragraph("Generated by: Krish Janwani & Associates (AI Assistant)")
    doc.add_paragraph("-" * 50)
    
    # 2. Advanced Cleaning Logic
    # Split text by double asterisks to find what should be bold
    parts = text.split('**')
    p = doc.add_paragraph()
    
    for i, part in enumerate(parts):
        # Even-indexed parts are normal text, odd-indexed parts were inside **
        if i % 2 == 0:
            # Clean up other markdown symbols like ### or ---
            clean_part = part.replace('###', '').replace('---', '')
            p.add_run(clean_part)
        else:
            # This makes the text BOLD in Word without the ** symbols
